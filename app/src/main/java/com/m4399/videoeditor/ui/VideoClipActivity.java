...
import top.jplayer.oblffmpeglib.CmdList;
...
public class VideoClipActivity extends AppCompatActivity implements OnPlayerEventListener,
    ...
    public void startClipVideo(View view) {
        showProgressDialog();
        VideoEditor.cropVideo(videoPath, mStartTime, mEndTime, new CmdInvoke.OnCmdExecListener() {
            @Override
    ...
    public void startChangeVideo(View view) {
        mCmd = new CmdList();
...
//         ffmpeg -y -i /storage/emulated/0/1/input.mp4 -f image2 -r 1 -q:v 10 -preset superfast
        mCmd.append("ffmpeg");
        mCmd.append("-y");
        mCmd.append("-i").append(videoPath);
        mCmd.append("-f").append("image2");
        mCmd.append("-r").append("1");
        mCmd.append("-q:v").append("10");
        mCmd.append("-preset").append("superfast");
        new Thread(new Runnable() {
            @Override
            public void run() {
                execCmd(mCmd, 100, new CmdInvoke.OnCmdExecListener() {
                    @Override
                    public void onSuccess() {
                        Log.e("asds", "success");
                    }
