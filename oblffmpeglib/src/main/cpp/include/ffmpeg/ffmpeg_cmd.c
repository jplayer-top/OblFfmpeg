...
void callJavaMethod(JNIEnv *env, jclass clazz,int ret) {
    ...
    //获取方法ID (I)V指的是方法签名 通过javap -s -public CmdInvoke 命令生成
...
Java_top_jplayer_oblffmpeglib_CmdInvoke_exec(JNIEnv *env, jclass clazz, jint cmdnum, jobjectArray cmdline)
{
...
Java_top_jplayer_oblffmpeglib_CmdInvoke_exit(JNIEnv *env, jclass type)
{
